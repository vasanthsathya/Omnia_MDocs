...
**Underlay Settings** are available by clicking the **Operations** navigation tab. Underlay Settings are used to manage the time and conditions under which network devices detect, respond to, and stabilize after changes in network topology or connection states. In technical networking terms, these could be broadly described as "Network Convergence Timing Controls" - the settings that determine how fast and how precisely a network can detect, communicate, and stabilize after any type of network state change.

![](media/operations_fabric.png)

### General Underlay Configuration
